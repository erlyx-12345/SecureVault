...
class _ProfileViewState extends State<ProfileView> {
  ...
  void initState() {
    ...
    // If registration just occurred, the route will carry the initial
    // first/last/email values so we can display them immediately.
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // Initialize ProfileViewModel afterwards so it can overwrite with
      // persisted data if available.
    ...
    return Scaffold(
      ...
      body: Consumer<ProfileViewModel>(
        builder: (context, profileVM, _) {
          // Update controllers when user data is loaded or changes.
          // This covers both initial load and navigation from registration.
          if (profileVM.user != null) {
            final fn = profileVM.firstName;
            final ln = profileVM.lastName;
            final em = profileVM.email;
            final b = profileVM.bio ?? '';
            if (_firstNameController.text != fn ||
                _lastNameController.text != ln ||
                _emailController.text != em ||
                _bioController.text != b) {
